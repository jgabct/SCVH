...
public class AdministrativeSector implements Initializable{
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		colRule.setCellValueFactory(cellData -> {
			try {
				return new SimpleStringProperty(((Sector) cellData.getValue()).getRuleCode().getKey());
			} catch (Exception e) {
				return new SimpleStringProperty("Sem Regra");
			}
		});
		
		ObservableList<Sector> teamMembers = FXCollections.observableArrayList(Main.getDatabase().getTableSector());
